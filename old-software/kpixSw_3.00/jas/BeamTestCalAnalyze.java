...
public class BeamTestCalAnalyze extends AbstractLoopListener implements RecordListener {

    boolean TestHits = false;
    int n_Sensors = 9;
    ...
    int nprinted = 0;
    int nprinted_max = 25;
    boolean[][][] bad_pixels = new boolean[n_Sensors][1024][4];
    double[] Sensor_Energy = new double[32];
    double[] Sensor_Hits_inTime = new double[32];
    IHistogram1D _Coincidence_N_Hist;
    ...
    IHistogram1D[] _CalibGainChi2Hist;
    IHistogram1D[] _CalibBaseChi2Hist;
    IHistogram1D[] _CalibInterceptHist;
    IHistogram1D[] _CalibInterceptBaseDifferenceHist;
    ICloud1D _Total_Deposited_Energy_Hist;
    IHistogram1D[] _BaseHist;
 ...
 //   IHistogram1D[] _inTime_HitsBySensor_Hist;
    ...
    public BeamTestCalAnalyze() {
        ...
        _Coincidence_N_Hist = hf.createHistogram1D("Coincidence Order", n_Sensors, 1, n_Sensors + 1);
        _isBad_Calibration_Hist = new IHistogram1D[4];
        _Cut_CalibGainHist = new IHistogram1D[n_Sensors];
        _CalibGainHist = new IHistogram1D[n_Sensors];
        _CalibGainChi2Hist = new IHistogram1D[n_Sensors];
        _CalibBaseChi2Hist = new IHistogram1D[n_Sensors];
        _CalibInterceptHist = new IHistogram1D[n_Sensors];
        _BaseHist = new IHistogram1D[n_Sensors];
 ...
 //       _inTime_HitsBySensor_Hist = new IHistogram1D[n_Sensors];
        ...
        _BunchTriggerDiff_Hist = new IHistogram1D[4];
       _BunchTime_Hist = new IHistogram1D[4][n_Sensors];

        tree.mkdir("/Calibrations");
        tree.cd("/Calibrations");
        for (int i = 0; i < n_Sensors; i++) {
            _CalibGainHist[i] = hf.createHistogram1D("Calib Gain Sensor " + i, 200, 1e15, 10e15);
            _CalibInterceptHist[i] = hf.createHistogram1D("Calibration Intercept Sensor " + i, 100, 0, 500);
            _CalibGainErrHist[i] = hf.createHistogram1D("Calibrated Gain Error Sensor " + i, 100, 0., 10e13);
            _CalibInterceptBaseDifferenceHist[i] = hf.createHistogram1D("Calibration Intercept - Base Difference Sensor " + i, 100, -10, 90);
            _BaseHist[i] = hf.createHistogram1D("Base Fit Mean Sensor " + i, 100, 0, 500);
            _Cut_CalibGainHist[i] = hf.createHistogram1D("Cut Calibration Gain Sensor " + i, 200, 1e15, 10e15);
            _CalibGainChi2Hist[i] = hf.createHistogram1D("Gain Chi sq Sensor " + i, 100, 0, 500);
            _CalibBaseChi2Hist[i] = hf.createHistogram1D("Base Chi sq Sensor " + i, 100, 0, 100);
        }
  ...
  //          _inTime_HitsBySensor_Hist[k] = hf.createHistogram1D("inTime Hits of Sensor " + k, 100, 0, 99);
        ...
        tree.mkdir("/Bunch Times");
        tree.cd("/Bunch Times");
        for (int k = 0; k < 4; k++) {
            _BunchTriggerDiff_Hist[k] = hf.createHistogram1D("Bunch - Trigger Time Bucket " + k, 201, -100, 100);
            _isBad_Calibration_Hist[k] = hf.createHistogram1D("Count of isBad Calibration Pixels - Bucket " + k, 32, 0, 31);
            for (int l = 0; l < n_Sensors; l++){
            _BunchTime_Hist[k][l] = hf.createHistogram1D("Bunch Number Bucket " + k +"Sensor " + l, 400,0,1312);
        }
        }
    }
    ...
    public void recordSupplied(RecordEvent re) {
        ...
        //
        //Count hits in this Sample, require less than cut if Test_Hits is true.
        //
        int nhits = 0;
        int[] nhitsBySensor;
...
// Come here once per pixel and bucket for all sensors
 ...
 /*           for (int i = 0; i < N_Triggers; i++) {
                ...
                _BunchTriggerDiff_Hist[KPiX_Bucket].fill(Bunch_Trigger_Time);
...
*/
            trigger_ok = true;
            if (!bad_pixels[Sensor_Address][KPiX_Channel][KPiX_Bucket]) {
                KpixCalibration calibration = sample1.getCalibration();
                if (calibration == null || calibration.isBad()) {
                    Cal_Amplitude = Double.NaN;
                    System.out.printf("Calibration null or isbad Sensor %d Channel %d  Bucket %d\n", Sensor_Address, KPiX_Channel, KPiX_Bucket);
                } else {
  ...
  //              _inTime_HitsBySensor_Hist[j].fill(Sensor_Hits_inTime[j]);
                ...
                if (Sensor_Hits_inTime[j] > 0) {
                    Coincidence_N++;
                }
                if (Coincidence_N > 0) {
                    _Coincidence_N_Hist.fill(Coincidence_N);
                }
            }
...
/*        double[] coincidence_count = new double[n_Sensors];
        ...
        for (int j = 0; j < n_Sensors; j++) {
            System.out.printf("Coincidence order %d = %d\n", j + 1, _Coincidence_N_Hist.binEntries(j));
        }
        ...
        for (int j = 1; j < n_Sensors - 1; j++) {
            double bad_pixel_count = _isBad_Calibration_Hist[0].binEntries(j);       
...
*/
    ...
    private void CalibrationAnalysis(KpixDataRecord data) {
        ...
        // convention is that bad_pixels will be true if pixel should not be used and   _isBad_Calibration_Hist will be filled.
        for (int j = 0; j < n_Sensors; j++) {
            for (int k = 0; k < 1023; k++) {
                for (int l = 0; l < 4; l++) {
                    bad_pixels[j][k][l] = false;
                }
            }
        }
        ...
        for (sensor_index = 0; sensor_index < n_Sensors; sensor_index++) {
            for (calIndex = 0; calIndex < 1024; calIndex++) {
                for (KPiX_Bucket = 0; KPiX_Bucket < 4; KPiX_Bucket++) {
                    try {
                        KpixCalibration cal = calSet.getCalibration(sensor_index, calIndex, KpixSample.ADCRange.NORMAL, KPiX_Bucket);
                        ...
                        double gainChi2 = cal.getDoubleParameter("CalibChisquare");
                        double baseChi2 = cal.getDoubleParameter("BaseFitChisquare");

                        if (cal.isBad() || cal == null) {
                            bad_pixels[sensor_index][calIndex][KPiX_Bucket] = true;
                            _isBad_Calibration_Hist[KPiX_Bucket].fill(sensor_index);
                        } else {
                            ...
                            if (KPiX_Bucket == 0) {
                                _BaseHist[sensor_index].fill(mean);
                                ...
                                _CalibGainChi2Hist[sensor_index].fill(gainChi2);
                                _CalibBaseChi2Hist[sensor_index].fill(baseChi2);
                                if (!bad_pixels[sensor_index][calIndex][KPiX_Bucket]) {
                        ...
                        bad_pixels[sensor_index][calIndex][KPiX_Bucket] = true;
                        _isBad_Calibration_Hist[KPiX_Bucket].fill(sensor_index);
                        // System.out.printf("Calibration Exception Sensor %d channel %d bucket %d\n ", sensor_index, calIndex, KPiX_Bucket);
                    }
                }
            }
        }
