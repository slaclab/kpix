...
int main (int argc, char **argv) {
   ...
   try {
      MultLink      multLink;
      MultDestPgp   * pgpDest;
      KpixControl   kpix(&multLink,defFile,32);
      int           pid;

      // Setup top level device
      kpix.setDebug(true);

      pgpDest = new MultDestPgp("/dev/pgpcard0");

      // Create and setup PGP link
      multLink.setMaxRxTx(500000);
      multLink.setDebug(true);
      multLink.open(1,pgpDest);
      multLink.enableSharedMemory("kpix",1);
      usleep(100);
